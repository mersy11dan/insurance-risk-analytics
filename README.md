...
DVC is used to version raw and processed data without storing large datasets in
Git. This keeps the repository lightweight while preserving an auditable record
of data inputs, processing steps, and generated outputs.

The project uses this simple data layout:

```text
data/
├── raw/                       # Original ACIS insurance dataset
│   └── insurance_data.csv
└── processed/                 # Cleaned data generated by DVC pipeline
    └── insurance_data_cleaned.csv
```

The DVC pipeline is defined in `dvc.yaml`. The current `prepare_data` stage runs
`src.data_preparation` to create a cleaned dataset from the raw insurance file.
The cleaning step is intentionally conservative: it trims text fields,
standardizes blank strings as missing values, and removes exact duplicate rows.

### Initialize DVC

If starting from a fresh clone without DVC metadata, initialize DVC from the
project root:
...
This creates the standard `.dvc/` project files. In this repository, DVC has
already been initialized.

### Add the Insurance Dataset to DVC

Place the original dataset under `data/raw/`:
...
Then add the dataset to DVC:

```bash
dvc add data/raw/insurance_data.csv
```

Track the DVC metadata with Git:

```bash
git add data/raw/insurance_data.csv.dvc .gitignore
```

The actual CSV remains outside Git and is stored in the DVC cache.

### Configure a Local DVC Remote

This project supports a simple local remote for audit-friendly storage:

```bash
dvc remote add -d localstorage ..\acis-dvc-storage
```

Push data objects to the local remote:

```bash
dvc push
```

The configured default remote is stored in `.dvc/config`.
